...
@Namespace("/sys")
@Component
public class ADept extends BaseAction implements ModelDriven<Dept> {
	@Autowired
	private BizService service;

	private long uid;
	private Dept bean = new Dept();

	@Action(value = "add2Dept", results = { @Result(name = "add2", location = "/WEB-INF/jsp/sys/addDept.jsp") })
	public String add2() {
		return "add2";
	}

	@Action(value = "getDept", results = { @Result(name = "getOne", location = "/WEB-INF/jsp/sys/modifyDept.jsp") })
	public String get() {
		try {
			Dept temp = (Dept) service.get(Dept.class, uid);
	...
	@Action(value = "deleteDept")
	public String delete() {
		try {
			service.delete(Dept.class, ids);
	...
	@Action(value = "queryDept", results = { @Result(name = "queryList", location = "/WEB-INF/jsp/sys/listDept.jsp") })
	public String query() {
		try {
			int pageNum = 0;
				...
				for (Object o : parm) {
					...
					if (name.startsWith("s_")) {
						...
						if (StringUtil.notEmpty(fieldValue)) {
							...
							parmnames.add(name);
							parmvalues.add(FieldUtil.format(Dept.class, name, fieldValue));
						}
					}
				}
			...
			Page page = null;
			page = service.find(p, Dept.class);
